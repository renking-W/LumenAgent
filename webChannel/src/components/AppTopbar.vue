<template>
  <header class="topbar">
    ...
    <div class="topbar-actions">
      ...
      <el-button v-else-if="activeView !== 'about' && activeView !== 'weixin'" type="primary" plain @click="$emit('refresh')">
        刷新数据
      </el-button>
...
const props = defineProps<{
  activeView: 'chat' | 'tools' | 'skills' | 'memories' | 'mcp' | 'vm' | 'config' | 'knowledge' | 'scheduler' | 'logs' | 'weixin' | 'about'
}>()
...
const pageTitle = computed(() => {
  ...
  if (props.activeView === 'weixin') return 'Weixin Channel'
...
const pageSubtitle = computed(() => {
  ...
  if (props.activeView === 'weixin') return '绑定个人微信，将消息接入完整的 Agent、记忆、工具与技能系统。'
